...
export const AnimatedBadge = ({
  ...
  const content = (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative flex max-w-fit items-center justify-center gap-3 rounded-full border border-slate-200 bg-white px-4 py-1.5 text-slate-700 transition-colors shadow-sm"
    >
      ...
      <span className="bg-clip-text text-xs font-bold uppercase tracking-widest">{text}</span>
  ...
  return (
    <>
      {href ? (
        <a href={href} className="inline-block no-underline mb-6">
          {content}
        </a>
      ) : (
